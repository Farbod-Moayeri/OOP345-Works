...
int main(int argc, char** argv)
{
	...
	if (argc == 5) {
		...
		std::string fileParser{};

		if (file)
		{
			for (auto i = 0; i < 4; i++)
			{
				std::getline(file, fileParser, '\n');
				if (fileParser[0] != '#')
				{
					library += sdds::Book(fileParser);
				}
				else
				{
					i--;
		...
		for (auto i = 0; file.good(); i++)
		{
			std::getline(file, fileParser, '\n');
			if (fileParser[0] != '#')
			{
				sdds::Book temp(fileParser);
	...
	if (argc > 2) {
		...
		if (file)
		{
			for (auto i = 0; i < 5; i++)
			{
				...
				if (fileParser[0] != '#')
				{
					movies[i] = sdds::Movie(fileParser);
				}
				else
				{
					i--;
